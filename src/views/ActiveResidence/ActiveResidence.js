...
function FiltersWrap({
  ...
  const dashboardCategoryEle = (
    <Switch
      checked={localFilters?.residenceCategory !== "WITHOUT_GCC"}
      onChange={(e) => {
        if (e) {
          setTimeout(() => {
            setLocalFilters((v) => ({
              ...v,
              residenceCategory: "WITH_GCC"
            }))
          }, 0)
        } else {
          setTimeout(() => {
...
function ActiveResidence({ nationalitiesConfigValueObj, emiratesConfigValue, residencyTypeValues }) {
  ...
  const firstRowEle = () => (
    <Row gutter={[getResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      <Col span={getResponsive({ default: 12, tablet: 24 })}>
        <ResidentsByGender isRtl={isRtl} filters={filters} />
      </Col>

      <Col span={getResponsive({ default: 12, tablet: 24 })}>
        <ResidentsByResidencyType allTypes={filtersData?.residencyTypes} isRtl={isRtl} filters={filters} />
      </Col>
    </Row>
  )
  ...
  const secondRowEle = () => (
    <Row gutter={[getResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      <Col span={getResponsive({ default: 12, tablet: 24 })}>
        <ResidentsByAge isRtl={isRtl} filters={filters} />
      </Col>

      <Col span={getResponsive({ default: 12, tablet: 24 })}>
        <ResidentsByEmirate emiratesConfigValue={filtersData?.emiratesConfigValue} filters={filters} isRtl={isRtl} />
      </Col>
    </Row>
  )
  ...
  const fourthRowEle = () => (
    <Row gutter={[getResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM }), themeVariables?.token?.marginSM]}>
      <Col span={getResponsive({ default: 12, tablet: 12, midTablet: 24 })}>
        <ResidentsByProfession isRtl={isRtl} filters={filters} />
      </Col>

      <Col span={getResponsive({ default: 12, tablet: 12, midTablet: 24 })}>
        <ResidentsBySponser isRtl={isRtl} filters={filters} />
      </Col>
    </Row>
  )
      ...
      <Row gutter={[0, getResponsive({ default: themeVariables?.token?.marginLG, desktop: themeVariables?.token?.marginSM })]}>
        ...
        {showAppliedFilters && (
          <Col>
            <AppliedFilters
              data={[
                ...(![undefined, null]?.includes(cleanedObject?.residenceCategory)
                  ? [
                    {
                      label: intl?.formatMessage({ id: "Include GCC Residents" }),
                      value: cleanedObject?.residenceCategory === "WITHOUT_GCC" ? intl?.formatMessage({ id: "No" }) : intl?.formatMessage({ id: "Yes" }),
