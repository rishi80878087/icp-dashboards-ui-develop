...
function ResidentsByProfession({ filters, isRtl }) {
  const intl = useIntl();
  const getResponsive = useResponsive();
  ...
  useEffect(() => {
    execute({ filters: { ...filters, profession_name: "" } })
  }, [filters]);
    ...
    <DashboardCard
      title={
        <Row align="middle" gutter={4}>
          ...
          <Col flex="none">
            <Tooltip
              title={intl?.formatMessage({ id: "active_residence_residency_sector_tooltip" })}
            >
              <span>
