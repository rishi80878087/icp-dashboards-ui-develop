...
import { getVisaHoldersByAgeRange } from "@/services/activeResidenceService";
...
function ResidentsByAge({ filters, isRtl }) {
  ...
  const {
    ...
    status,
    value,
  } = useAsync({ asyncFunction: getVisaHoldersByAgeRange });
  ...
  const { values, categories } = (sortedData || []).reduce((acc, val) => {
    acc.values?.[0]?.data?.push(val?.total_movements);
    ...
    <DashboardCard
      ...
      title={
        <Row align="middle" gutter={4}>
          <Col flex="none">
            {intl?.formatMessage({ id: "Visa Holders By Age" })}
          </Col>
          <Col flex="none">
            <Tooltip
              title={intl?.formatMessage({ id: "visa_holders_age_tooltip" })}
            >
              <span>
