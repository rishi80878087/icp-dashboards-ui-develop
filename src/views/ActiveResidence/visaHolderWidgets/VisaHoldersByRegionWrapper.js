...
import { getVisaHoldersByEmirates, getVisaHoldersByRegion } from "@/services/activeResidenceService";
...
function RisksByNationality({
  ...
  } = useAsync({ asyncFunction: getVisaHoldersByRegion });
  ...
  const minMaxValues = useMemo(() => {
    let min = 0;
    let max = 0;
    let total_movements = 0;
    let totalEntries = 0;
    let totalExits = 0
    if (_data?.length) {
      min = _data?.[0]?.total_movements;
      max = _data?.[0]?.total_movements;
      _data?.forEach((v) => {
        total_movements += v?.total_movements;
        totalEntries += v?.entries;
        totalExits += v?.exits;

        if (v?.total_movements < min) {
          min = v?.total_movements;
        }

        if (v?.total_movements > max) {
          max = v?.total_movements;
        }
      })
    }
    return { min, max, total_movements, totalEntries, totalExits };
      ...
      const json = data?.map((v) => {
        ...
        return ({
          ...
          color: getColorFromPercentage({
            percent: ((v?.total_movements / minMaxValues.max) * 100),
          }),
          label: isRtl ? v?.nationality_ar : v?.nationality_en,
          value: v?.total_movements,
          entries: v?.entries,
          exits: v?.exits,
          ...item
        })
        ...
        <Table
          ...
          columns={[
            ...
            },
            {
              ...
              render: (v) => {
                const percent = Number(((v?.value / minMaxValues.total_movements) * 100)?.toFixed(1));
          ...
          footer={() => (
            <Table
              ...
              columns={[
                ...
                {
                  width: getResponsive({ default: "34%" }),
                  title: formatNumber(minMaxValues?.total_movements) || "-",
                }
              ]}
            />
      ...
      <DashboardCard
        ...
        title={
          <Row align="middle" gutter={4}>
            <Col flex="none">
              {intl?.formatMessage({ id: "Visa Holders by Region & Country " })}
            </Col>
            <Col flex="none">
              <Tooltip
                title={intl?.formatMessage({ id: "visa_holders_nationality_tooltip" })}
              >
                <span>
                        ...
                        <Tabs
                          ...
                          options={(emiratesList || [])?.map((emirate) => ({
                            ...
                            children: <RegionWrap
                              ...
                              label={isRtl ? emirate?.emirate_ar : emirate?.emirate_en}
                              value={formatNumber(emirate?.total_movements)}
                            />
                          }))}
                        />
                      )
